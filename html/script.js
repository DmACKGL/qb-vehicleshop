...
window.addEventListener('message', function(event) {
	...
	if (data.type == "successful-buy") {
		...
		$('#messagePopup').append(`
        <span>You bought: ` + data.vehicleName + ` for ` + data.value + `</span>
        `);
		setTimeout(function() {
	...
	if (data.type == "updateVehicleInfos") {
		...
		$('#contentVehicle').append(`
            <div class="row spacebetween" id="vehicleinfoprice">
                <span class="priceVehicle">$` + discountPrice + `</span>
            </div>
        ...
        if (vehicleSelected.discount > 0 || vehicleSelected.discount < 0 ) {
            ...
            <span class="priceVehicle" style="text-decoration:line-through">$` + originalPrice + `</span>
            `);
        };
...
function menuVehicle(event) {
	...
	document.getElementById("carouselCars").innerHTML = '';
	dataVehicles = [];
	$.post('https://qb-vehicleshop/menuSelected', JSON.stringify({
		menuId: categoryVehicleSelected.toLowerCase()
	}));
}

function testDrive() {
	$.post('https://qb-vehicleshop/TestDrive', JSON.stringify({
...
function openModalMenu() {
	...
	$('#closemenu').append(`
        ...
        <div class="modal-footer">
            <div class="modal-price" id="modalprice">
                <p class='price sale'>$` + discountPrice + `</p>
            </div>
    ...
    if (vehicleSelected.discount > 0 || vehicleSelected.discount < 0 ) {
        $('#modalprice').append(`
            <p class='price discount'>$`+ originalPrice +`</p>
        `);
    };
}
...
$(document).on('keydown', function() {
	switch (event.keyCode) {
		...
		case 17: // TAB
			break;
		case 68: // LEFT A
			$.post('https://qb-vehicleshop/rotate', JSON.stringify({
				key: "left"
			}))
			break;
		case 65: // RIGHT D
...
$(document).on('mousedown', ".item-cars", function(event) {
	switch (event.which) {
		...
		case 1:
			...
			$(event.currentTarget).addClass('selectedVehicle').queue(function(next) {
			...
			if (!itemDisabled) {
				...
				document.getElementById("vehiclebrand").innerHTML = '';
				document.getElementById("contentVehicle").innerHTML = '';
				document.getElementById("vehiclebrand").innerHTML = ' <img id="vehicle_brand_image" src="../imgs/' + dataCar[0].outerText + '.png">';
				$('#nameBrand').append(`
				...
				$.post("https://qb-vehicleshop/SpawnVehicle", JSON.stringify({
					modelcar: dataCar[9].outerText,
                ...
                <div class="item-cars">
                    <div class="col-lg-3 col-md-6 ">
                        <div class="specification" style="opacity:0.0; position:absolute;">
                            ...
                            <span id="discount">` + data[i].discount + `</span>
                        </div>
                        <div class="img-fluid" style="background-image:  url(../imgs/` + modelUper + `.png);"> <span class="exibeName">` + data[i].name + `</span></div>
...
function setVehicleColorRGB(R, G, B) {
	if (selectedColor == 'primary') {
		$.post("https://qb-vehicleshop/RGBVehicle", JSON.stringify({
			primary: true,
			R: R,
			G: G,
			B: B
